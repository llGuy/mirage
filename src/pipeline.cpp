...
void pso_config::enable_blending_same(uint32_t attachment_index, VkBlendOp op, VkBlendFactor src, VkBlendFactor dst) 
{
...
void pso_config::configure_vertex_input(uint32_t attrib_count, uint32_t binding_count) 
{
...
void pso_config::set_binding(uint32_t binding_idx, uint32_t stride, VkVertexInputRate input_rate) 
{
...
void pso_config::set_binding_attribute(uint32_t location, uint32_t binding, VkFormat format, uint32_t offset) 
{
...
void pso_config::set_default_values_() 
{
  ...
  /* Dynamic states */
  dynamic_states_ = heap_array<VkDynamicState>({ VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_SCISSOR });
